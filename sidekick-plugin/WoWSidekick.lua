...
-- PAYLOAD BUILDER
-- =========================
-- Encodes all available game state into 39 data bits.
-- This is the maximum capacity for a 40-box grid with 1 sync bit.
--
-- Bit Layout (39 bits total):
--  [0-6]   : Player HP % (0-127)
--  [7-13]  : Target HP % (0-127)
--  [14-20] : Player Resource % (mana/energy/rage 0-127)
--  [21-25] : Distance to Target (0-31 yards)
--  [26]    : In Combat (bool)
--  [27]    : Has Target (bool)
--  [28-32] : Player Level (0-31)
--  [33-37] : Target Level (0-31)
--  [38-39] : Movement State (0=ground, 1=mounted, 2=flying)
local function buildPayload()
    ...
    -- Player Resource % (7 bits) - mana/energy/rage/focus
    local resource = 0
    ...
    -- Distance to Target (5 bits: 0-31 yards)
    local distance = 0
    if UnitExists("target") then
        -- Use CheckInteractDistance to estimate range
        for i = 1, 4 do
            if CheckInteractDistance("target", i) then
                distance = i * 5  -- rough estimate
                break
            end
        end
        if distance == 0 then distance = 31 end  -- out of range
    end
